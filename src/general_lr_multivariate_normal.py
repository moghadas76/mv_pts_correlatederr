...
class GeneralLowRankMultivariateNormal(Distribution):
    r"""
    ...
    def __init__(self, loc, cov_factor, cov_diag, corr_mat, corr_eye, reg_w, validate_args=None):
        if loc.dim() < 1:
    ...
    def expand(self, batch_shape, _instance=None):
        new = self._get_checked_instance(GeneralLowRankMultivariateNormal, _instance)
        ...
        super(GeneralLowRankMultivariateNormal, new).__init__(batch_shape,
                                                       self.event_shape,
                                                       validate_args=False)
    ...
    def log_prob(self, value):
        ...
        diff = value - self.loc  # (1, DB)
        M = _batch_lowrank_mahalanobis(self._unbroadcasted_cov_factor,  # (1, DB, DR)
                                       self._unbroadcasted_cov_diag,  # (1, DB)
                                       diff,
                                       self._capacitance_tril)  # (1, DR, DR)
